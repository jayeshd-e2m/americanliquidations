...
<section class="py-24 bg-gray <?php echo esc_attr($block_class); ?>" <?php if ($block_id): ?>id="<?php echo esc_attr($block_id); ?>"<?php endif; ?>>
	<div class="container">
		<div class="flex gap-8"> 
			<div class="w-1/2 bg-white border-r-8 border-primary p-10 inline-flex items-center rounded-l-[15px]">
				<blockquote>
			...
			<div class="w-1/2">
				...
				<div class="flex items-center gap-10 mt-12">
					...
					if( $link ): 
						...
						$link_target = $link['target'] ? $link['target'] : '_self';
						?>
						<a class="btn-link text-black hover:text-primary text-sm font-bold tracking-[0.10em] uppercase" href="<?php echo esc_url( $link_url ); ?>" target="<?php echo esc_attr( $link_target ); ?>"><?php echo esc_html( $link_title ); ?></a>
					<?php endif; ?>
				</div>
				<div class="testimonial-blocks mt-24">
					<div class="testimonial-items flex gap-4">
					<?php
