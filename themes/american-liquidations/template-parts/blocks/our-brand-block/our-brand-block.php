				...
				if( have_rows('our_brand_blocks') ):
					while( have_rows('our_brand_blocks') ) : the_row(); ?>
						<div class="brand-item group">
							<a href="<?php echo get_field('brand_url'); ?>" class="brand-item-img h-[148px] bg-gray flex items-center justify-center rounded-2xl p-3 mb-6">
								<?php 
								$image = get_sub_field('brand_image');
								if( !empty( $image ) ): ?>
									<img class="transition-transform duration-300 ease-in-out group-hover:scale-105" style="max-width: 90%; max-height: 90%;" src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
								<?php endif; ?>
							</a>
							<h6 class="text-black/60"><a href="<?php echo get_field('brand_url') ?>"><?php echo get_sub_field('brand_title'); ?></a></h6>
