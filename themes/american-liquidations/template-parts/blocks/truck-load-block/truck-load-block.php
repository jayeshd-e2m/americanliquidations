...
<section class="bg-gray py-16 lg:py-24 <?php if(!$section_visible){ ?> hidden <?php } ?><?php echo esc_attr($block_class); ?>" <?php if ($block_id): ?>id="<?php echo esc_attr($block_id); ?>"<?php endif; ?>>
	<div class="container">
		...
		if ($new_arrival_query->have_posts()) {
			echo '<div class="product-has-slider grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mt-12">';
			...
			echo '<p class="mt-8 text-center">No Truck Load products available at the moment.</p>';
		}
		?>
